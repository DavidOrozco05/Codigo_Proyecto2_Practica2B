...
char cola_add(eCola* c,DATOCOLA dato)
{
    ...
    // los mas seguro ser?a apagarlas ac?!
    // cli();//arduinito
    ...
    // si llego hasta ac? es por que la cola no estaba 
...
DATOCOLA cola_get(eCola* c)
{
    ...
    {
        return ERROR;// esto dependeria de la aplicaci?n reservar un caracter para el error. tambien se puede usar una bandera en la estructura cola
    }
    // si llego hasta ac? es por que la cola no estaba 
    // llena y pide meter el dato en la cola
    if (c->salida >=TAM)
